...
#define REFLECT_PARENT(A)\
self->parents.push_back(A::reflection);\
...
#define REFLECT_PROPERTY(N)\
self->properties.push_back(registration::variant(#N, typeid(N).name(), offsetof(T, N), sizeof(N)));\

#define REFLECT_FUNCTION(F)\
self->functions.push_back(registration::method{#F, &T::F});\
...
namespace registration
{
  ...
  struct property
  {
    ...
    property(std::string n, std::string t, unsigned o, unsigned s, void* b)
      : name(n), type(t), offset(o), size(s), base(b)
    {}
  ...
  struct Node
  {
    ...
    Node(std::string name)
      : key(name), properties(), functions(), Vtable(false)
    {}
  };
  ...
  struct class_ : public Node
  {
    ...
    std::vector<variant> get_properties()
    {
      return properties;
    }

    std::vector<method> get_functions()
    {
      return functions;
    }
    std::vector<class_> get_parents()
    {
      return parents;
    }
  };
